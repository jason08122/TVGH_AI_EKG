...
class MLP(nn.Module):
    def __init__(self, in_dim: int = 2, out_dim: int = 1, device: str = "cpu") -> None:
        super().__init__()
...
class SimRvLvDysIden(nn.Module):
    def __init__(self, nums_tabular: int = 2) -> None:
        super().__init__()
        self.backbone = EfficientNet.from_name('efficientnet-b0', in_channels=12, num_classes=64)
        self.mlp = MLP(in_dim=nums_tabular)

        in_dim = self.backbone._fc.out_features + 1
        self.fc1 = nn.Sequential(nn.Linear(in_dim, 128), nn.ReLU())
        self.fc2 = nn.Sequential(nn.Linear(128, 64), nn.ReLU())
        self.fc3 = nn.Sequential(nn.Linear(64, 32), nn.ReLU())
        self.fc4 = nn.Sequential(nn.Linear(32, 16), nn.ReLU())

        self.final_layer = nn.Sequential(nn.Linear(16, 3), nn.Softmax(dim=0))
...
if __name__ == '__main__':
    ...
    # device = "cuda:0"
    x = torch.randn(16, 12, 2500).to(device)
    # y = torch.randn(64, 2).to(device)
    # model = SimRvLvDysIden(nums_tabular=2).to(device)
    model = EfficientNet.from_name('efficientnet-b0', in_channels=12, num_classes=64).to(device)
    # a, b = model(x)
    # print(b.shape)

    # inputs = torch.rand(16, 12, 2500)
    # print(x.shape)
    # model = EfficientNet.from_name('efficientnet-b4', in_channels=12, num_classes=64)
